...
impl Model for St7735r {
        ...
        DI: display_interface::WriteOnlyDataCommand,
    {
        match rst {
            ...
            None => {
                dcs.write_command(mipidsi::dcs::SoftReset).map_err(|_| InitError::DisplayError)?;
                delay.delay_ms(150);
            }
        }

        dcs.write_command(mipidsi::dcs::ExitSleepMode).map_err(|_| InitError::DisplayError)?;
        delay.delay_ms(500);

        // Power & Frame Rate
        dcs.write_raw(0xB1, &[0x01, 0x2C, 0x2D]).map_err(|_| InitError::DisplayError)?;
        dcs.write_raw(0xB2, &[0x01, 0x2C, 0x2D]).map_err(|_| InitError::DisplayError)?;
        dcs.write_raw(0xB3, &[0x01, 0x2C, 0x2D, 0x01, 0x2C, 0x2D]).map_err(|_| InitError::DisplayError)?;
        dcs.write_raw(0xB4, &[0x07]).map_err(|_| InitError::DisplayError)?;

        // Power Control
        dcs.write_raw(0xC0, &[0xA2, 0x02, 0x84]).map_err(|_| InitError::DisplayError)?;
        dcs.write_raw(0xC1, &[0xC5]).map_err(|_| InitError::DisplayError)?;
        dcs.write_raw(0xC2, &[0x0A, 0x00]).map_err(|_| InitError::DisplayError)?;
        dcs.write_raw(0xC3, &[0x8A, 0x2A]).map_err(|_| InitError::DisplayError)?;
        dcs.write_raw(0xC4, &[0x8A, 0xEE]).map_err(|_| InitError::DisplayError)?;
        dcs.write_raw(0xC5, &[0x0E]).map_err(|_| InitError::DisplayError)?;

        // Gamma
        dcs.write_raw(0xE0, &[0x0f, 0x1a, 0x0f, 0x18, 0x2f, 0x28, 0x20, 0x22, 0x1f, 0x1b, 0x23, 0x37, 0x00, 0x07, 0x02, 0x10]).map_err(|_| InitError::DisplayError)?;
        dcs.write_raw(0xE1, &[0x0f, 0x1b, 0x0f, 0x17, 0x33, 0x2c, 0x29, 0x2e, 0x30, 0x30, 0x39, 0x3f, 0x00, 0x07, 0x03, 0x10]).map_err(|_| InitError::DisplayError)?;
    ...
    fn write_pixels<DI, I>(&mut self, dcs: &mut Dcs<DI>, colors: I) -> Result<(), display_interface::DisplayError>
    where
        DI: display_interface::WriteOnlyDataCommand,
        I: IntoIterator<Item = Self::ColorFormat>,
    {
        dcs.write_command(mipidsi::dcs::WriteMemoryStart).map_err(|_| display_interface::DisplayError::BusWriteError)?;
        for color in colors {
            let bytes = color.to_be_bytes();
            dcs.di.send_data(display_interface::DataFormat::U8(&bytes))?;
        }
        Ok(())
    }
}
    ...
    I2C: embedded_hal::i2c::I2c,
{
    ...
    fn read_als_lux(&mut self) -> Result<f32, I2C::Error> {
        ...
        let ratio = if (ch0 + ch1) > 0.0 { (ch1 * 100.0) / (ch1 + ch0) } else { 101.0 };
        let (c0, c1) = if ratio < 45.0 { (17743.0, -11059.0) } else if ratio < 64.0 { (42785.0, 19548.0) } else if ratio < 85.0 { (5926.0, -1185.0) } else { (0.0, 0.0) };
...
fn main() -> ! {
    ...
    let spi = Spi::new(peripherals.SPI2, SpiConfig::default().with_frequency(Rate::from_mhz(4)).with_mode(Mode::_0))
        .unwrap().with_sck(sck).with_mosi(mosi);
    ...
    let i2c_bus = I2c::new(peripherals.I2C0, I2cConfig::default())
        .unwrap().with_sda(peripherals.GPIO19).with_scl(peripherals.GPIO18);
    ...
    let mut uart = Uart::new(peripherals.UART1, UartConfig::default().with_baudrate(9600))
        .unwrap().with_tx(peripherals.GPIO16).with_rx(peripherals.GPIO17);
    ...
    let outline_style = PrimitiveStyleBuilder::new().stroke_color(Rgb565::BLUE).stroke_width(1).build();
    ...
    loop {
        ...
        fn calc_res(raw: u16) -> f32 {
            if raw == 0 { return 0.0; }
            if raw >= 4095 { return 100_000_000.0; } 
        ...
        display.clear(Rgb565::BLACK).unwrap();
        Rectangle::new(Point::new(0, 0), Size::new(160, 80)).into_styled(outline_style).draw(&mut display).unwrap();

        match page {
            0 => {
                // Climate Page
                Text::new("1/4 CLIMATE", Point::new(45, 12), title_style).draw(&mut display).unwrap();
                
                let mut buf = [0u8; 32];
                Text::new("Temp:", Point::new(5, 30), label_style).draw(&mut display).unwrap();
                let _ = core::fmt::write(&mut format_wrapper::FormatWrapper(&mut buf), format_args!("{:.2}", data.temp));
                let val_str = core::str::from_utf8(&buf).unwrap().trim_matches('\0');
                Text::new(val_str, Point::new(45, 30), data_style).draw(&mut display).unwrap();
                Text::new("C", Point::new(45 + (val_str.len() as i32 * 6) + 2, 30), label_style).draw(&mut display).unwrap();

                let mut buf = [0u8; 32];
                Text::new("Humid:", Point::new(5, 45), label_style).draw(&mut display).unwrap();
                let _ = core::fmt::write(&mut format_wrapper::FormatWrapper(&mut buf), format_args!("{:.2}", data.hum));
                let val_str = core::str::from_utf8(&buf).unwrap().trim_matches('\0');
                Text::new(val_str, Point::new(45, 45), data_style).draw(&mut display).unwrap();
                Text::new("%", Point::new(45 + (val_str.len() as i32 * 6) + 2, 45), label_style).draw(&mut display).unwrap();

                let mut buf = [0u8; 32];
                Text::new("Press:", Point::new(5, 60), label_style).draw(&mut display).unwrap();
                let _ = core::fmt::write(&mut format_wrapper::FormatWrapper(&mut buf), format_args!("{:.2}", data.press));
                let val_str = core::str::from_utf8(&buf).unwrap().trim_matches('\0');
                Text::new(val_str, Point::new(45, 60), data_style).draw(&mut display).unwrap();
                Text::new("hPa", Point::new(45 + (val_str.len() as i32 * 6) + 2, 60), label_style).draw(&mut display).unwrap();
                
                info!("Temperature: {} °C, Humidity: {} %, Pressure: {} hPa", data.temp, data.hum, data.press);
            }
            1 => {
                // Light Page
                Text::new("2/4 LIGHT", Point::new(45, 12), title_style).draw(&mut display).unwrap();
                
                let mut buf = [0u8; 32];
                Text::new("Lux:", Point::new(5, 35), label_style).draw(&mut display).unwrap();
                let _ = core::fmt::write(&mut format_wrapper::FormatWrapper(&mut buf), format_args!("{:.2}", data.lux));
                let val_str = core::str::from_utf8(&buf).unwrap().trim_matches('\0');
                Text::new(val_str, Point::new(45, 35), data_style).draw(&mut display).unwrap();
                Text::new("Lux", Point::new(45 + (val_str.len() as i32 * 6) + 4, 35), label_style).draw(&mut display).unwrap();

                let mut buf = [0u8; 32];
                Text::new("Prox:", Point::new(5, 55), label_style).draw(&mut display).unwrap();
                let _ = core::fmt::write(&mut format_wrapper::FormatWrapper(&mut buf), format_args!("{}", data.prox));
                Text::new(core::str::from_utf8(&buf).unwrap().trim_matches('\0'), Point::new(45, 55), data_style).draw(&mut display).unwrap();
                
                info!("Light: {} Lux, Proximity: {}", data.lux, data.prox);
            }
            2 => {
                // Gas Page - ADC Flush + Cached Display
                Text::new("3/4 GASES", Point::new(45, 12), title_style).draw(&mut display).unwrap();
                ...
                let draw_ohm_local = |label: &str, y: i32, val: f32, display: &mut _| {
                    let mut buf = [0u8; 32];
                    Text::new(label, Point::new(5, y), label_style).draw(display).unwrap();
                    let unit: &str;
                    if val >= 1_000_000.0 {
                        let m = val / 1_000_000.0;
                        let _ = core::fmt::write(&mut format_wrapper::FormatWrapper(&mut buf), format_args!("{}.{}", m as u32, ((m % 1.0) * 10.0) as u32));
                        unit = "M-O";
                    } else if val >= 1_000.0 {
                        let k = val / 1_000.0;
                        let _ = core::fmt::write(&mut format_wrapper::FormatWrapper(&mut buf), format_args!("{}.{}", k as u32, ((k % 1.0) * 10.0) as u32));
                        unit = "K-O";
                    } else {
                        let _ = core::fmt::write(&mut format_wrapper::FormatWrapper(&mut buf), format_args!("{}", val as u32));
                        unit = "Ohms";
                    }
                    let val_str = core::str::from_utf8(&buf).unwrap().trim_matches('\0');
                    Text::new(val_str, Point::new(45, y), data_style).draw(display).unwrap();
                    Text::new(unit, Point::new(45 + (val_str.len() as i32 * 6) + 4, y), label_style).draw(display).unwrap();
                };
            ...
            3 => {
                ...
                Text::new("4/4 PARTICLES", Point::new(45, 12), title_style).draw(&mut display).unwrap();
                Text::new("SYNCING...", Point::new(45, 45), label_style).draw(&mut display).unwrap();
                ...
                if let Some(f) = last_f {
                    ...
                    Rectangle::new(Point::new(1, 15), Size::new(158, 64)).into_styled(PrimitiveStyleBuilder::new().fill_color(Rgb565::BLACK).build()).draw(&mut display).unwrap();
                    
                    info!("PM1.0: {} μg/m³, PM2.5: {} μg/m³, PM10: {} μg/m³", f.pm1_0, f.pm2_5, f.pm10);
                    let mut buf = [0u8; 32];
                    Text::new("PM1.0:", Point::new(5, 30), label_style).draw(&mut display).unwrap();
                    let _ = core::fmt::write(&mut format_wrapper::FormatWrapper(&mut buf), format_args!("{}", f.pm1_0));
                    let val_str = core::str::from_utf8(&buf).unwrap().trim_matches('\0');
                    Text::new(val_str, Point::new(45, 30), data_style).draw(&mut display).unwrap();
                    Text::new("ug", Point::new(45 + (val_str.len() as i32 * 6) + 2, 30), label_style).draw(&mut display).unwrap();

                    let mut buf = [0u8; 32];
                    Text::new("PM2.5:", Point::new(5, 45), label_style).draw(&mut display).unwrap();
                    let _ = core::fmt::write(&mut format_wrapper::FormatWrapper(&mut buf), format_args!("{}", f.pm2_5));
                    let val_str = core::str::from_utf8(&buf).unwrap().trim_matches('\0');
                    Text::new(val_str, Point::new(45, 45), data_style).draw(&mut display).unwrap();
                    Text::new("ug", Point::new(45 + (val_str.len() as i32 * 6) + 2, 45), label_style).draw(&mut display).unwrap();

                    let mut buf = [0u8; 32];
                    Text::new("PM10:", Point::new(5, 60), label_style).draw(&mut display).unwrap();
                    let _ = core::fmt::write(&mut format_wrapper::FormatWrapper(&mut buf), format_args!("{}", f.pm10));
                    let val_str = core::str::from_utf8(&buf).unwrap().trim_matches('\0');
                    Text::new(val_str, Point::new(45, 60), data_style).draw(&mut display).unwrap();
                    Text::new("ug", Point::new(45 + (val_str.len() as i32 * 6) + 2, 60), label_style).draw(&mut display).unwrap();
                }
            }
            _ => page = 0,
        }
